...
function shoot(e, word, next) {
  ...
  if (used.includes(word)) {
    if (word === 'loop' && clearRoute.every((value, index) => used[index] === value) && used.length === clearRoute.length) {
      loopClosed = true; used.push(word); locked = true; mark(e); pending = 'P'; message.textContent = 'ガチャ… ドアの向こうへ進める。'; door.classList.add('open'); door.focus(); clearSound(); updateTrail(); return;
    }
    used = []; loopClosed = false;
  }
  used.push(word); locked = true; mark(e); pending = next; message.textContent = 'ガチャ… ドアの向こうへ進める。'; door.classList.add('open'); door.focus(); unlockSound();
...
door.addEventListener('click', () => {
  if (pending) { current = pending; render(); return; }
...
function updateTrail() { trail.textContent = used.length ? used.map(word => word.toUpperCase()).join('  →  ') : '—'; }
